...
def inbound_email():
    """
    Cloud Function endpoint for processing inbound emails.
    Expected payload from email gateway is JSON with keys: sender, to, cc, subject, body.
    """
    try:
        ...
        # Extract mailing list keyword from the "to" address (before the @)
        list_keyword = to_addr.split("@")[0]

        logging.info(f"Processing email from {sender} to list {list_keyword} with subject '{subject}'.")

        # Check for subscribe command (e.g., "3 days")
        ...
        if subscribe_match:
            ...
            # Subscribe any CC addresses
            if cc_list:
                for cc in cc_list:
                    subscribe_user(list_keyword, cc, duration)
            # Send confirmation email
            confirmation_msg = f"You are now subscribed to {list_keyword} for {duration} day(s)."
            send_email(sender, f"Subscription confirmed for {list_keyword}", confirmation_msg)
            return jsonify({"status": "subscribed"}), 200

        # Check for unsubscribe command
        if subject == "unsubscribe":
            unsubscribe_user(list_keyword, sender)
            send_email(sender, f"Unsubscribed from {list_keyword}", "You have been unsubscribed.")
        ...
        if extend_match:
            ...
            send_email(sender, f"Subscription extended for {list_keyword}", f"Your subscription has been extended by {extension} day(s).")
            return jsonify({"status": "extended"}), 200

        # Check for help/info command
        if subject in ["help", "info"]:
            send_help_email(sender)
            return jsonify({"status": "help_sent"}), 200

        # Otherwise, relay the email to all subscribers anonymously
        ...
        # Anonymize the email content (strip sender info, remove headers, etc.)
        relay_body = anonymize_email_content(email_data)

        # For each subscriber (except the sender), send the relayed email
        for sub in subscribers:
            if sub.get("email") != sender:
                # Generate an unsubscribe link for each recipient
                unsubscribe_link = generate_unsubscribe_link(sub.get("email"), list_keyword)
                relay_body_with_unsub = relay_body + f"\n\nUnsubscribe: {unsubscribe_link}"
                send_email(sub.get("email"), f"[{list_keyword}] New message", relay_body_with_unsub)
...
def cleanup():
    """
    Cloud Function endpoint triggered by Cloud Scheduler to remove expired subscriptions.
    """
...
def unsubscribe_link():
    """
    HTTP endpoint to handle unsubscribe link clicks.
